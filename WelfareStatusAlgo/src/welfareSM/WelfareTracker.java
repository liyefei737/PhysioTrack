...
public class WelfareTracker {
	...
	public WelfareTracker(List<Integer> customHRRange, List<Integer> customBRRange, List<Float> customSTRange,
			List<Float> customCTRange) {
		initStateMaps();
		wap = new WelfareAlgoParams(customHRRange, 0.25, customBRRange, 0.25, customSTRange, 0.25, customCTRange, 0.25);
	}

	public WelfareTracker(List<Integer> customHRRange, double heartWeight, List<Integer> customBRRange,
			double breathWeight, List<Float> customSTRange, double skinWeight, List<Float> customCTRange,
			double coreWeight) {
		initStateMaps();
		wap = new WelfareAlgoParams(customHRRange, heartWeight, customBRRange, breathWeight, customSTRange, skinWeight,
				customCTRange, coreWeight);
	}
	...
	public void setPhysioParamThresholds(List<Integer> heartRateRange, List<Integer> breathRateRange,
			List<Float> skinTempRange, List<Float> coreTempRange) {
	...
	public Object[] calculateWelfareStatus(JSONArray lastXsecondsData) {
		...
		if (numValidParams == 0) {
			...
			float stateAverage = (float) (wap.getBrWeight() * currentStateMap.get(BR).ordinal()
					+ wap.getCtWeight() * currentStateMap.get(CT).ordinal()
					+ wap.getHrWeight() * currentStateMap.get(HR).ordinal()
					+ wap.getStWeight() * currentStateMap.get(ST).ordinal());

			nextState = stateAverage < 1.5 ? GREEN : (stateAverage < 2.5 ? YELLOW : RED);
		}
	...
	private HashMap<String, Object> processJSONDataArray(JSONArray lastXsecondsData) {
		// TODO: look at actual time length of data, 15 sec intervals for hr and
		// br
